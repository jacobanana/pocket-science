...
import { renderGridSVG } from './render.js'
...
app.innerHTML = `
  ...
  that honors offsets, ghosts and swing. Follow a path to hear how one groove became the next.</p>
...
function renderAll(){
  viewEl.innerHTML = `
    ...
    <div id="list"></div>`
  const searchEl = viewEl.querySelector('#search'), chapEl = viewEl.querySelector('#chapter'),
        sortEl = viewEl.querySelector('#sort'), countEl = viewEl.querySelector('#count'),
        listEl = viewEl.querySelector('#list')
  const chapters = [...new Set(BANK.patterns.map(p=>p.chapter||'—'))]
  chapEl.innerHTML += chapters.map(c=>`<option>${c}</option>`).join('')

  function render(){
    stopPlayback()
    const q = (searchEl.value||'').toLowerCase(), ch = chapEl.value, sort = sortEl.value
  ...
  chapEl.addEventListener('change', render)
...
function renderPathIndex(){
  viewEl.innerHTML = `
    ...
    Pick a path and walk a lineage groove by groove, with notes on exactly what changes at each step.</p>
    <div class="pathgrid">${PATHS.map(pa=>`
      <button class="pathcard" data-path="${pa.id}">
        <h3>${pa.title}</h3>
        <p class="stops">${pa.steps.length} stops</p>
        <p>${pa.blurb}</p>
      </button>`).join('')}
    </div>`
  viewEl.querySelectorAll('[data-path]').forEach(b =>
    b.addEventListener('click', () => { location.hash = `#/paths/${b.dataset.path}` }))
}

function renderPath(pa){
  viewEl.innerHTML = `
    <button class="backbtn" id="back">← all paths</button>
    <div class="pathintro">
      <h2>${pa.title}</h2>
      <p>${pa.intro}</p>
    </div>
    <div id="steps"></div>`
  viewEl.querySelector('#back').addEventListener('click', () => { location.hash = '#/paths' })
  const stepsEl = viewEl.querySelector('#steps')
  pa.steps.forEach((s, i) => {
    ...
    row.innerHTML = `
      <div class="steprail">
        ...
        ${i < pa.steps.length-1 ? '<div class="stepline"></div>' : ''}
      </div>
      <div class="stepbody">
...
function route(){
  stopPlayback()
  const hash = location.hash || '#/paths'
  const m = hash.match(/^#\/(paths|all|guide)(?:\/([\w-]+))?/)
  const view = m ? m[1] : 'paths'
  ...
  if(view==='all') renderAll()
    ...
    const pa = m && m[2] ? PATHS.find(p=>p.id===m[2]) : null
    if(pa) renderPath(pa); else renderPathIndex()
  }
  window.scrollTo(0, 0)
}
